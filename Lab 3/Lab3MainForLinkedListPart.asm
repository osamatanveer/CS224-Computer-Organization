...
removeEnd:
	...
	addi $a1, $a1, -1 
forForRemoveEnd:
	beq $t7, $a1, atPositionOfEnd
	move $t5, $t6
	lw $t6, 0($t6) # update curr
	addi $t7, $t7, 1
	j forForRemoveEnd
atPositionOfEnd:	
	sw $zero, 0($t5)
	j delete_n_done																																					
	# to remove after head
forToGetToN:
	beq $t1, $t0, reachedAtN # i to n
	move $t2, $t3 # moving curr into prev
	lw $t3, 0($t3) # updating curr
	addi $t1, $t1, 1
	j forToGetToN	
...
forToDisplayRecursively:
	addi $sp, $sp, -8
	sw $t0, 0($sp)
	sw $ra, 4($sp)
	bne $t0, $zero, makeResucriveCall
	addi $sp, $sp, 8
	jr $ra
...
msg115:  .asciiz "4 - invert the linked list  \n"

msg116:  .asciiz "5 - delete element at position n from linked list \n"

msg117:  .asciiz "6 - recursively display list in reverse \n"
...
msg133: .asciiz "The linked list displayed in reverse is " 
